...
onready var HitBox2 = get_node("AttackArea/HitBox2")
onready var HitBox3 = get_node("AttackArea/HitBox3")
...
func _process(delta):
	if dead:
		...
		if(!$DownRay.is_colliding() or $RightRay.is_colliding() or $RightRay2.is_colliding() or $RightRay3.is_colliding()):
			var collider = $RightRay.get_collider()
			var collider2 = $RightRay2.get_collider()
			var collider3 = $RightRay3.get_collider()
			if (collider and collider.name == "Player") or (collider2 and collider2.name == "Player") or (collider3 and collider3.name == "Player"):
...
func _on_AnimatedSprite_frame_changed():
	if($AnimatedSprite.animation == "ataque" and $AnimatedSprite.frame >= 2 and $AnimatedSprite.frame <= 4):
